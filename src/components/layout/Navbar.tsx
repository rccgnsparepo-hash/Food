...
import { ShoppingBag, MapPin, User, Shield, Bike, UtensilsCrossed } from 'lucide-react';
  ...
  return (
    <header className="sticky top-0 z-40 bg-white/85 backdrop-blur-md border-b border-rose-100/80 shadow-2xs transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
        ...
        {/* Address Selector (Desktop/Tablet) */}
        <motion.div
          whileHover={{ y: -1, scale: 1.01 }}
          whileTap={{ scale: 0.98 }}
          className="hidden md:flex items-center gap-2 bg-rose-50/80 border border-rose-100 rounded-full px-4 py-2 text-xs font-semibold text-slate-700 cursor-pointer hover:bg-rose-100/60 transition-colors shadow-2xs"
        >
          <MapPin className="w-4 h-4 text-[#D6001C]" />
          <span>Deliver to:</span>
          <span className="text-slate-900 font-bold truncate max-w-[200px]">
            {user?.address || 'Prayer City, Mountain Top University'}
          </span>
        </motion.div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="bg-slate-100/90 p-1 rounded-full flex items-center gap-1 text-xs font-semibold border border-slate-200/50">
            {[
              { id: 'customer', label: 'Order', icon: UtensilsCrossed, targetView: 'home' },
              { id: 'rider', label: 'Rider', icon: Bike, targetView: 'rider' },
              { id: 'admin', label: 'Admin', icon: Shield, targetView: 'admin' },
            ].map((r) => {
              const Icon = r.icon;
